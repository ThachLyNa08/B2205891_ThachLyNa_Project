...
import { useAuthStore } from '@/stores/auth'; // Dùng @ cho an toàn
  ...
  routes: [
    ...
    // === KHU VỰC USER (PUBLIC) ===
    {
      ...
      children: [
        ...
        { path: 'history', name: 'history', component: MyLoansView, meta: { requiresAuth: true } },
      ]
    },
...
router.beforeEach((to, from, next) => {
  ...
  if (to.path === '/' && isAuthenticated) {
     if (['admin', 'staff'].includes(userRole)) return next({ name: 'admin-dashboard' });
